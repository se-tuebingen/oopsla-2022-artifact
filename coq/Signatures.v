...
(** [get] returns the value bound to the given label in an environment
    or [None] if the given label is not bound.  If the label has
...
(** An environment is well-formed if and only if each label is bound at
    most once. *)

Inductive ok : list (label * A) -> Prop :=
  | ok_nil :
      ok nil
  | ok_cons : forall (E : list (label * A)) (x : label) (a : A),
...
Lemma cons_concat : forall (A : Type) (E : list (label * A)) x a,
...
Lemma dom_singleton_list : forall (A : Type) (x : label) (a : A),
...
Lemma ok_push :  forall (E : list (label * A)) (x : label) (a : A),
      ok E -> ~ In x (dom E) -> ok ([(x, a)] ++ E).
Proof.
  exact (@ok_cons A).
Qed.
...
(** A binding in the middle of an environment has an label fresh from
...
Lemma binds_singleton : forall x a,
  binds x a [(x,a)].
Proof.
  intros x a. unfold binds. simpl. destruct (eq_label_dec x x); intuition.
Qed.
...
Proof with auto.
  ...
  destruct (eq_label_dec x y)... intros _ J. destruct J. lsetdec.
Qed.
...
Proof.
  ...
  discriminate.
  destruct (eq_label_dec x y); intuition.
Qed.
...
Proof.
  unfold binds. simpl. intros. destruct (eq_label_dec x y).
    split; congruence.
    discriminate.
Qed.
...
Proof with auto.
  ...
  destruct (eq_label_dec x x); intuition.
  inversion Ok; subst. destruct (eq_label_dec x z)...
    destruct H3. simpl_env. lsetdec.
Qed.
...
Proof with auto.
  ...
  destruct (eq_label_dec z z). congruence. intuition.
  inversion Ok; subst. destruct (eq_label_dec z x)...
    destruct H4. simpl_env. lsetdec.
Qed.
...
Proof.
  ...
  binds_cases H.
  binds_cases H; subst. auto using union_3. lsetdec.
Qed.
...
Proof.
  ...
  binds_cases H.
  binds_cases H. intuition. lsetdec.
Qed.
...
Proof.
  ...
  auto.
  inversion Ok; subst. destruct (eq_label_dec x y); subst; auto.
